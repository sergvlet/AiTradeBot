...
console.log("📈 strategy-chart.js loaded (dashboard modular, FIXED v5.0)");

// =============================================================
// 🔥 НОВОЕ! STOMP WebSocket для Strategy Live Events
// =============================================================
let strategyStomp = null;

// Подписка на события стратегий
function subscribeStrategyLive(chatId, strategyType) {
    if (strategyStomp) {
        try { strategyStomp.deactivate(); } catch {}
        strategyStomp = null;
    }
    ...
    client.debug = () => {}; // отключаем лог в консоль

    client.connect({}, () => {
        const topic = `/topic/strategy/${strategyType}/${chatId}`;
        console.log("🔌 Subscribed strategy live:", topic);

        client.subscribe(topic, msg => {
            let ev;
            try { ev = JSON.parse(msg.body); }
            catch { return; }

            handleStrategyEvent(ev);
        });
    });

    strategyStomp = client;
}


// -------------------------------------------------------------
// 🔥 Обработчик всех событий стратегий
// -------------------------------------------------------------
function handleStrategyEvent(ev) {
    console.log("📨 Strategy event:", ev);
    ...
    switch (ev.type) {
        ...
        case "LEVELS":
            drawFiboLevels(ev.levels);
            break;

        case "STATE":
            updateStateLabel(ev.state);
            break;
        ...
        case "METRIC":
            updateMetricPanel(ev.pnlPct);
            break;

        case "SIGNAL":
            showSignal(ev.signal, ev.value);
            break;
    }
    function handleLiveCandle(ev) {
        if (!ev.open || !ev.close) return;
        ...
        if (!last || last.time < t) {
            candlesGlobal.push(stateC);
        } else {
            candlesGlobal[candlesGlobal.length - 1] = stateC;
        }
...
// -------------------------------------------------------------
// 🔥 UI обновления Live-панелей
// -------------------------------------------------------------
function updateStateLabel(active) {
    const el = document.getElementById("state-label");
    if (!el) return;

    el.textContent = active ? "Активна" : "Остановлена";
    el.style.color = active ? "#2ecc71" : "#e74c3c";
}

function updateMetricPanel(pnl) {
    const el = document.getElementById("pnl-metric");
    if (!el) return;
    el.textContent = (pnl * 100).toFixed(2) + "%";
}

function showSignal(name, value) {
    const box = document.getElementById("signal-box");
    if (!box) return;

    box.innerHTML = `<b>${name}</b>: ${value.toFixed(4)}`;
    box.style.opacity = 1;

    setTimeout(() => box.style.opacity = 0.6, 2000);
}

// -------------------------------------------------------------
// 🔥 Отрисовка уровней Фибоначчи (если есть)
// -------------------------------------------------------------
let fiboSeries = [];

function clearFiboLevels() {
    fiboSeries.forEach(s => chart.removeSeries(s));
    fiboSeries = [];
}

function drawFiboLevels(levels) {
    if (!chart) return;
    ...
    levels.forEach(level => {
        ...
        s.setData([{ time: candlesGlobal[0].time/1000, value: level },
            { time: candlesGlobal.at(-1).time/1000, value: level }]);
...
// =============================================================
// ГЛОБАЛЬНОЕ СОСТОЯНИЕ (ТВОЙ КОД, НЕ ТРОГАЛ)
// =============================================================
let currentWs = null;
