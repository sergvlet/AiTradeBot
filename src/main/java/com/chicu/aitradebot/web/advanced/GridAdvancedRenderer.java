// src/main/java/com/chicu/aitradebot/web/advanced/GridAdvancedRenderer.java
package com.chicu.aitradebot.web.advanced;

import com.chicu.aitradebot.common.enums.StrategyType;
import com.chicu.aitradebot.domain.enums.AdvancedControlMode;
import com.chicu.aitradebot.strategy.grid.GridStrategySettings;
import com.chicu.aitradebot.strategy.grid.GridStrategySettingsService;
...
public class GridAdvancedRenderer implements StrategyAdvancedRenderer {

    private final GridStrategySettingsService settingsService;
    ...
    public String render(AdvancedRenderContext ctx) {
        ...
        GridStrategySettings s = settingsService.getOrCreate(ctx.getChatId());
        ...
        return ""
                ...
                + "    <div class='fw-bold'>GRID — параметры стратегии</div>"
                +      badge(ro)
                + "  </div>"
                + "  <div class='row g-3'>"
                + fieldNumber("gridLevels", "Уровни сетки", valInt(s.getGridLevels()),
                    "min='1' step='1'", dis, roAttr, "Количество уровней сетки.")
                + fieldNumber("gridStepPct", "Шаг сетки (%)", valBd(s.getGridStepPct()),
                    "min='0' step='0.01'", dis, roAttr, "Шаг между уровнями в процентах.")
                + fieldNumber("orderVolume", "Объём ордера", valBd(s.getOrderVolume()),
                    "min='0' step='0.00000001'", dis, roAttr, "Объём одной заявки (как трактует исполнение).")
                + "  </div>"
                + hint(ro)
                + "</div>";
    }
    ...
    public void handleSubmit(AdvancedRenderContext ctx) {
        if (ctx.getControlMode() == AdvancedControlMode.AI) {
            log.info("🔒 GRID advanced ignored (AI mode)");
            return;
        }

        Map<String, String> p = ctx.getParams();

        GridStrategySettings incoming = GridStrategySettings.builder()
                .chatId(ctx.getChatId())
                .gridLevels(parseInt(p.get("gridLevels")))
                .gridStepPct(parseBd(p.get("gridStepPct")))
                .orderVolume(parseBd(p.get("orderVolume")))
                .build();

        settingsService.update(ctx.getChatId(), incoming);
        log.info("✅ GRID advanced saved (chatId={})", ctx.getChatId());
    }
